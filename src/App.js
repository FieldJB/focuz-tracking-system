...
const appId =
  typeof __app_id !== "undefined" ? __app_id : "focuz-mes-production";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 1. Subscribe to the Master Board Roster
    ...
    const unsubBoards = onSnapshot(
      ...
      (snapshot) => {
        const activeBoards = [];
        snapshot.forEach((doc) => activeBoards.push(doc.data()));
    ...
    // 2. Subscribe to the Traceability Ledger
    ...
    const unsubTx = onSnapshot(
      ...
      (snapshot) => {
        const txHistory = [];
        snapshot.forEach((doc) => txHistory.push(doc.data()));
  ...
  const metrics = useMemo(() => {
    return {
      ...
      completed: boards.filter((b) => b.currentStage === STAGES.COMPLETED)
        .length,
      highRework: [...boards]
        .sort((a, b) => b.reworkCycles - a.reworkCycles)
        .slice(0, 5),
    };
  }, [boards]);
  ...
  const activeReworkBoards = useMemo(() => {
    return boards
      ...
      .map((board) => {
        const defectTx = [...transactions].find(
          (tx) => tx.sn === board.sn && tx.to === STAGES.REWORK
        );
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (event) => {
      ...
      if (rows.length < 2) {
        showToast(
          "QUALITY ALERT: File appears empty or missing headers.",
          "error"
        );
        return;
      }
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        let board = boards.find((b) => b.sn === rawSn) || {
          sn: rawSn,
          product: "Prysm",
          currentStage: STAGES.SMT,
          reworkCycles: 0,
        };
        ...
        if (bulkAction === "Transfer to Prysm") {
          toStage = STAGES.PRYSM;
          actionLabel = "Transferred";
        } else if (bulkAction === "Return for Rework") {
          toStage = STAGES.REWORK;
          actionLabel = "Defect Found (Bulk)";
          ...
          toStage = STAGES.COMPLETED;
          actionLabel = "Passed Test";
        } else if (bulkAction === "Receive from Rework") {
          toStage = STAGES.PRYSM;
          actionLabel = "Rework Completed";
        }
        ...
        promises.push(setDoc(boardDocRef, board));
        promises.push(setDoc(txDocRef, newTx));
        successCount++;
      }
      ...
      try {
        ...
        showToast(
          `Upload Complete: ${successCount} boards routed. ${errorCount} rejected formats.`
        );
      } catch (err) {
        console.error(err);
  ...
  const handleScanSubmit = async (e) => {
    ...
    let board = boards.find((b) => b.sn === scanSn) || {
      sn: scanSn,
      product: "Prysm",
      currentStage: STAGES.SMT,
      reworkCycles: 0,
    ...
    const newTx = {
      ...
      location:
        scanAction === "Return for Rework" ? scanLocation || "N/A" : "N/A",
    };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
            <Activity size={20} className="text-blue-500" /> Live Cloud
            Movements
          </h3>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
            <AlertTriangle size={20} className="text-orange-500" /> High Rework
            Attention
          </h3>
            ...
            <div className="px-6 py-4 bg-red-50 border-b border-red-100 flex justify-between items-center">
              <div>
                ...
                <p className="text-red-500 text-sm font-medium mt-1">
                  Current WIP: {activeReworkBoards.length} Units pending repair
                  in SMT
                </p>
              </div>
              <button
                ...
                <tbody>
                  {activeReworkBoards.length === 0 ? (
                    <tr>
                      ...
                      >
                        No boards currently in rework queue.
                      </td>
                    </tr>
                  ) : (
        ...
        {uploadMode === "manual" ? (
          <>
            <div className="text-center mb-8">
              ...
              <p className="text-gray-500 text-sm mt-1">
                Data saves directly to centralized database
              </p>
            </div>
          ...
          <div className="text-center py-6">
            ...
            <p className="text-gray-500 text-sm mb-6 px-4">
              Select an action below and upload a <b>.CSV</b> file to mass-route
              boards globally.
            </p>
            ...
            <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl p-8 mb-6 relative hover:bg-gray-100 transition-colors">
              ...
              <div className="flex flex-col items-center justify-center pointer-events-none">
                <span className="text-gray-600 font-bold mb-2">
            ...
            <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg text-left text-sm">
              <h4 className="font-bold text-yellow-800 mb-2 flex items-center gap-1">
                <AlertTriangle size={16} /> Required CSV Format (Headers
                Optional)
              </h4>
              ...
              {filteredTx.length === 0 ? (
                <tr>
                  <td colSpan="7" className="text-center py-8 text-gray-400">
                    Database is empty. Upload a CSV to begin tracking.
                  </td>
                </tr>
              ) : (
                filteredTx.map((tx) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      <span
                        ...
                        ${
                          tx.action.includes("Defect")
                            ? "bg-red-100 text-red-700"
                            : tx.action.includes("Passed")
                            ? "bg-green-100 text-green-700"
                            : "bg-orange-100 text-orange-700"
                        }`}
                      >
                    ...
                    <td
                      className="p-3 text-gray-600 font-mono text-xs"
                      title={tx.user}
                    >
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans relative">
      ...
      {toast && (
        <div
          className={`fixed top-4 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-lg shadow-xl z-50 flex items-center gap-2 font-bold text-sm ${
            toast.type === "error"
      ...
      <nav className="w-full md:w-64 bg-white border-r border-gray-200 text-slate-800 flex flex-col shadow-sm z-10">
        <div className="p-6 border-b border-gray-100 flex flex-col items-start">
          ...
          <p className="text-orange-600 text-xs font-bold uppercase tracking-wider mt-1">
            Quality Control
          </p>
        </div>
        ...
        <div className="p-4 border-t border-gray-100 text-xs text-gray-500">
          ...
          <p className="font-mono bg-gray-100 p-1 rounded overflow-hidden text-ellipsis">
            {user?.uid || "Connecting..."}
          </p>
        </div>
      </nav>
